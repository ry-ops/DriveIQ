...
export default function ChatWidget() {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      const response = await chatApi.send([{ role: "user", content: userMessage.content }], sessionId);
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 && (
              <div className="text-center text-gray-500 text-sm py-8">
                ...
                <p className="text-xs mt-2 text-gray-400">Maintenance tips, how-to guides, and more</p>
              </div>
            )}
            {messages.map((msg) => (
          ...
          <div className="p-3 border-t border-gray-200">
            <form onSubmit={(e) => { e.preventDefault(); sendMessage(input); }} className="flex gap-2">
              <input
                ...
                disabled={isLoading}
                className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-toyota-red focus:border-transparent text-sm disabled:opacity-50"
              />
              <button type="submit" disabled={!input.trim() || isLoading} className="px-3 py-2 bg-toyota-red text-white rounded-md hover:bg-red-700 disabled:opacity-50 transition-colors">
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-4" onClick={() => setSelectedImage(null)}>
          <div className="relative max-w-4xl max-h-full">
            ...
            <img src={selectedImage} alt="Document page" className="max-w-full max-h-[80vh] object-contain rounded-lg" />
          </div>
        </div>
      )}
    </>
